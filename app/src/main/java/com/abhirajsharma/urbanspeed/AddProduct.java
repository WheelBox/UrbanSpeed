...
import com.abhirajsharma.urbanspeed.adapter.AddProductDescriptionAdapter;
import com.abhirajsharma.urbanspeed.adapter.AddproductImageAdapter;
import com.abhirajsharma.urbanspeed.model.AddProductDescriptionModel;
import com.abhirajsharma.urbanspeed.model.AddProductImageModel;
...
public class AddProduct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        previous_name = getIntent().getStringExtra("name");
        previous_tag = getIntent().getStringExtra("relevant_tag");
        if (code == 1) {
            final int[] PS = {0};
            ...
            DocumentReference ref = FirebaseFirestore.getInstance().collection("STORES").document(store_id).collection("PRODUCTS").document();
                    ...
                    .addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            if (task.isSuccessful()) {
                                ...
                                FirebaseFirestore.getInstance().collection("STORES").document(store_id).collection("PRODUCTS").document(product_id).update(Ima)
    ...
    private void addProduct(@org.jetbrains.annotations.NotNull String name, String price, String cutPrice, String offer, String stock, String home_desc, String relevanr_Tags) {
        ...
        if (name.isEmpty() || price.isEmpty() || cutPrice.isEmpty() || offer.isEmpty() || stock.isEmpty() || relevanr_Tags.isEmpty() || home_desc.isEmpty()) {
            ...
            FirebaseFirestore.getInstance().collection("STORES").document(store_id).collection("PRODUCTS").document(product_id).set(AddProductDetails)
                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            ...
                            if (task.isSuccessful()) {
                                ...
                                FirebaseFirestore.getInstance().collection("STORES").document(store_id).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                                    @Override
                                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                                        if (task.isSuccessful()) {
                                            String tag = task.getResult().get("relevant_tags").toString();
                                            String[] preVious_tags = tag.trim().split(",");
                                            ArrayList<String> l1 = new ArrayList<>(Arrays.asList(preVious_tags));
                                            ...
                                            Map<String, Object> AddProductDetailToStore = new HashMap<>();
                                            AddProductDetailToStore.put("relevant_tags", tag);
                                            AddProductDetailToStore.put("tags", l1);
                                            FirebaseFirestore.getInstance().collection("STORES").document(store_id).update(AddProductDetailToStore).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    if (task.isSuccessful()) {
                                                    }
    ...
    private void updateProduct(String name, String price, String cutPrice, String offer, String stock, int previousStock, String home_desc, String relevanr_Tags) {
        ...
        if (name.isEmpty() || price.isEmpty() || cutPrice.isEmpty() || offer.isEmpty() || relevanr_Tags.isEmpty() || home_desc.isEmpty()) {
            ...
            FirebaseFirestore.getInstance().collection("STORES").document(store_id).collection("PRODUCTS").document(product_id).update(AddProductDetails)
                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                                ...
                                FirebaseFirestore.getInstance().collection("STORES").document(store_id).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                                    ...
                                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            String[] previous_Product_tags = previous_tag.trim().split(",");
                                            ArrayList<String> previous_Product_tagsList = new ArrayList<>(Arrays.asList(previous_Product_tags));

                                            l1.removeAll(previous_Product_tagsList);
                                            l1.remove(previous_name);
